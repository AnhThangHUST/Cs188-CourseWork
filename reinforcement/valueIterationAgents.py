    ...
    def runValueIteration(self):
        ...
        for i in range(0,self.iterations):
            ...
            for state in states:
                ...
                # Ta khong the su dung dong code tren vi phai het 1 iteration ta moi duoc quyen update state
            self.values = V
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
            ...
            if not self.mdp.isTerminal(state): # for all non terminal states
                for action in self.mdp.getPossibleActions(state):
                    for nextState, probability in self.mdp.getTransitionStatesAndProbs(state, action):
                        if probability > 0:
                            # Luu lai state :(predecessor1, predecessor2)
                            predecessors[nextState].add(state)
        
        PQ = util.PriorityQueue()
        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                diff = abs(self.values[state] - getMaxQval(state))
                PQ.push(state, -diff)
        ...
        for i in range(self.iterations):
            ...
            state = PQ.pop()
            if not self.mdp.isTerminal(state):
                self.values[state] = getMaxQval(state)
        
            for p in predecessors[state]:
                diff = abs(self.values[p] - getMaxQval(p))
                if diff > self.theta:
                    PQ.update(p, -diff)
