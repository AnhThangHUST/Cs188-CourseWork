...
class QLearningAgent(ReinforcementAgent):
    ...
    #Action thi return None con Value thi return 0.0 if legalAction = []
    def computeValueFromQValues(self, state):
        ...
        if len(legalActions) == 0:
            return 0.0 
        listQValue = [self.getQValue(state, action) for action in legalActions]
        return max(listQValue)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feat in features:
            qValue += features[feat] * weights[feat]
        return qValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        diff = reward + self.discount * max_qValue - qValue
